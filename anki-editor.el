  ...
  (let* ((oldnote (thread-first
                      (anki-editor-api--with-multi
                       ...
                       (anki-editor-api--enqueue 'updateNoteFields
                                                 :note (anki-editor-api--note note)))
                    (caar)))
         (tags-add (thread-first (anki-editor-note-tags note)
                     (cl-set-difference (alist-get 'tags oldnote) :test 'string=)
                     (cl-set-difference anki-editor-ignored-org-tags :test 'string=)))
      ...
      (when (org-goto-first-child)
        (while (/= point-of-last-child (point))
          ...
          (let* ((inhibit-message t)  ;; suppress echo message from `org-babel-exp-src-block'
                 ...
                 (exporter (or (org-entry-get-with-inheritance anki-editor-prop-exporter) anki-editor-exporter-default))
            ...
            (setq raw-content (or (and contents-begin
                                  ...
                                  ""))
            (push (cons field-name (pcase exporter
...
of that heading."
  ...
  (let ((total (progn
    ...
    (message (cond
              ...
              ((zerop failed) (format "Successfully pushed %d notes to Anki" acc))
              (t (format "Pushed %d notes in total, among which %d were failed.  Check property drawers for failure reasons.
After have fixed the issues, you could re-push the failed ones with `anki-editor-retry-failed-notes'."
                         acc failed))))))
...
bugfixes or new features of AnkiConnect."
  (interactive)
  (when (yes-or-no-p "NOTE: This will download the latest AnkiConnect from the Internet and install to your computer,
which might not be safe or stable.  Do you still want to continue? ")
